...
if (!empty($pagina)) {
    ...
    <thead>
        ...
        <th class='text-center'>Data de Token</th>
        <th class='text-center'>Data de Pag</th>
    ...
    while ($row = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
        ...
        $viewToken = '<button class="btn btn-info btn-sm vtoken" data-id="' . $token . '" aria-hidden="true" style="margin-bottom: 10px;"><i class="fa fa-eye fa-1"></i></button>';

        // data-toggle="modal" data-target="#edittModal"
        $edit = ($_SESSION['usuario']['tipo'] != 2) ? '' : '<button class="btn btn-warning btn-sm etoken" data-id="' . $token . '" style="margin-bottom: 10px;"><span class="glyphicon glyphicon-trash"></span> Editar</button>';

        // data-toggle="modal" data-target="#deleteModal"
        $delette = ($_SESSION['usuario']['tipo'] != 2) ? '' : '<button class="btn btn-danger btn-sm deletec" data-id="' . $id_token . '" style="margin-bottom: 10px;"><span class="glyphicon glyphicon-trash"></span> Excluir</button>';
        ...
        $dados .=  "<tr>
            ...
            <td class='text-center' max-width='120px'>
                " . date_format(date_create($pag_resp), 'd/m/Y') . "
            </td>
